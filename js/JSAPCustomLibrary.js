(function(global, $) {
	...
	Light.prototype.randomKey = function(size) {
  ...
  return key;
	...
	Light.prototype.showChineseWeek = function(sDate, DispObjName) {
    var dispWeek = document.getElementById(DispObjName);
    var week = "";
    if (sDate != ""){
        week = "(" + getweek(CreateDate(sDate)) + ")";
    }
    dispWeek.innerHTML = week;
}
	function getweek(Date) {
    now = Date;
    if (now.getDay() == 0) return ("\u65E5");
    if (now.getDay() == 1) return ("\u4E00");
    if (now.getDay() == 2) return ("\u4E8C");
    if (now.getDay() == 3) return ("\u4E09");
    if (now.getDay() == 4) return ("\u56DB");
    if (now.getDay() == 5) return ("\u4E94");
    if (now.getDay() == 6) return ("\u516D");
}
function CreateDate(xDate) {
    // xDate:yyyy/mm/dd
    var xres = xDate.split("/");
    var xY = parseFloat(xres[0]);
    var xM = parseFloat(xres[1]) - 1;
    var xD = parseFloat(xres[2]);
    var newDate = new Date(xY, xM, xD);
    return newDate;
}
